...
export class AcpAgent {
  ...
  private async ensureAuggieAuth(): Promise<void> {
    if (this.extra.backend !== 'auggie') return;
    await this.ensureBackendAuth('auggie', 'login');
  }
  ...
  private async performAuthentication(): Promise<void> {
    try {
      ...
      } else if (this.extra.backend === 'auggie') {
        await this.ensureAuggieAuth();
      }
