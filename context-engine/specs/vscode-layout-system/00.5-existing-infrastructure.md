...
**Extend:** UI structure (modal → panel)
...
|-------|----------|---------|
| `activePanelId` | Context/Hook | Which panel is open ('conversations', 'workspace', 'preview', null) |
...
### REUSE (No Changes)
- ✅ `ChatHistory` component (wrap only)
- ✅ `ChatWorkspace` component (minor handler update)
- ✅ File loading logic in `WorkspaceFilePreview`
- ✅ Routing system
- ✅ Authentication/authorization

### EXTEND (Modify Existing)
- 🔧 `Layout` - Add panel system
- 🔧 `ChatLayout` - Remove workspace, add dynamic sizing
- 🔧 `WorkspaceFilePreview` - Convert modal to panel
- 🔧 `useSiderWidthWithDrag` - Adapt for new layout
- 🔧 Mobile responsiveness - Add overlay mode

### NET NEW (Create from Scratch)
- ⭐ `IconSidebar` component
- ⭐ `LeftPanel` container component
- ⭐ `ConversationPanel` wrapper
